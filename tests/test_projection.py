...
from pyfusion.utils import projection, structs


@pytest.fixture
def eigen_decomp() -> structs.EigenDecomp:
    eigvec_1 = np.array([[[1.0, 0.0], [0.0, 1.0]]])
    eigval_1 = np.array([[1.0, 2.0]])
    eigvec_2 = np.array([[[0.0, 1.0], [1.0, 0.0]]])
    eigval_2 = np.array([[3.0, 4.0]])

    return structs.EigenDecomp(
        eigen_1=structs.Eigen(eigvec=eigvec_1, eigval=eigval_1),
        eigen_2=structs.Eigen(eigvec=eigvec_2, eigval=eigval_2),
    )


@pytest.fixture
def eigen_decomp_3d() -> structs.EigenDecomp3D:
    ...
    return structs.EigenDecomp3D(
        eigen_1=structs.Eigen3D(eigvec=eigvec_1, eigval=eigval_1),
        eigen_2=structs.Eigen3D(eigvec=eigvec_2, eigval=eigval_2),
        eigen_3=structs.Eigen3D(eigvec=eigvec_3, eigval=eigval_3),
    )


def test_projection_calculate_eigen(eigen_decomp: structs.EigenDecomp):
...
def test_projection_calculate_eigen3d(eigen_decomp_3d: structs.EigenDecomp3D):
